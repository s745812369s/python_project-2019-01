...
# 2-1. 종목별 문화재 리스트 항목을 입력받았다면, 
    국보, 보물, 사적, 명승, 천연기념물 등의 종목을 선택하는 구문이 나오게 되고, 선택한 항목에 따라 전국에 있는 종목별 문화재 리스트를 보여줍니다. 리스트에 없는 것을 입력 했다면 다시 입력 받습니다.
# 2-2. 지역별 문화재 리스트 항목을 입력받았다면, 
    서울, 부산, 경기, 경남, 울산, 대구 등의 지역을 선택하는 구문이 나오게 되고, 선택한 항목에 따라 그 지역에 있는 문화재 리스트를 보여줍니다. 리스트에 없는 것을 입력 했다면 다시 입력 받습니다.
# 2-3. 문화재 이름 검색 항목을 입력받았다면, 
    안내받고 싶은 문화재 이름을 입력받고 문화재에 대한 한글 해설(종류, 이름, 위치, 설명)을 출력합니다.
...
3-1. 입력받은 문화재에 대하여, "선택한 언어의 나레이션을 들으시겠습니까? (Y/N)로 대답"에 대한 답을 입력 받고, 대답에 따라 선택한 언어의 나레이션 음성을 재생합니다. 
